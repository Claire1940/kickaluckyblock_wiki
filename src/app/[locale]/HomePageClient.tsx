...
export default function HomePageClient({ latestArticles, moduleLinkMap, locale, homeExternalLinks }: HomePageClientProps) {
  const t = useMessages() as any
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.kickaluckyblock.wiki'
  const { playRoblox, joinDiscord, developerGroup, youtubeVideo } = homeExternalLinks.links
  const { id: featuredVideoId, title: featuredVideoTitle } = homeExternalLinks.video
    ...
    '@graph': [
      {
        ...
        description: 'Kick a Lucky Block Wiki covering codes status, brainrots, mutations, weights, zones, rebirth routes, and beginner tips for the Roblox tycoon.',
        image: {
          '@type': 'ImageObject',
          url: `${siteUrl}/images/hero.webp`,
          width: 1920,
          height: 1080,
          caption: 'Kick a Lucky Block Wiki',
        },
        potentialAction: {
          '@type': 'SearchAction',
      ...
      },
      {
        ...
        name: 'Kick a Lucky Block Wiki',
        alternateName: 'Kick a Lucky Block',
        url: siteUrl,
        description: 'Unofficial community wiki for Kick a Lucky Block with verified gameplay references and update tracking.',
        logo: {
          '@type': 'ImageObject',
        ...
        image: {
          ...
          width: 1920,
          height: 1080,
          caption: 'Kick a Lucky Block Wiki hero image',
        },
        sameAs: [
          playRoblox,
          developerGroup,
      ...
      },
      {
        ...
        genre: ['Simulation', 'Tycoon', 'Roblox'],
        publisher: {
          '@type': 'Organization',
          name: 'No More Flops',
          url: developerGroup,
        ...
        <div className="container mx-auto max-w-5xl">
          ...
          <div className="scroll-reveal grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {t.modules.lucidBlocksCreaturesAndEnemies.creatures.map((c: any, index: number) => (
              <div key={index} className="p-6 bg-white/5 border border-border rounded-xl hover:border-[hsl(var(--nav-theme)/0.5)] transition-colors">
                <div className="mb-3">
                  <span className={`text-xs px-2 py-1 rounded-full border ${["Hostile Enemy","Major Threat","Elite Threat"].includes(c.role) ? "bg-[hsl(var(--nav-theme)/0.14)] border-[hsl(var(--nav-theme)/0.45)] text-[hsl(var(--nav-theme-light))]" : "bg-[hsl(var(--nav-theme)/0.1)] border-[hsl(var(--nav-theme)/0.3)]"}`}>{c.role}</span>
                </div>
                <h3 className="font-bold mb-2">
            ...
            {t.modules.lucidBlocksBestEarlyUnlocks.priorities.map((p: any, index: number) => (
              <div key={index} className="p-6 bg-white/5 border border-border rounded-xl hover:border-[hsl(var(--nav-theme)/0.5)] transition-colors">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full border ${p.priority === "Essential" ? "bg-[hsl(var(--nav-theme)/0.14)] border-[hsl(var(--nav-theme)/0.45)] text-[hsl(var(--nav-theme-light))]" : p.priority === "Very High" ? "bg-orange-500/10 border-orange-500/30 text-orange-400" : "bg-[hsl(var(--nav-theme)/0.1)] border-[hsl(var(--nav-theme)/0.3)]"}`}>{p.priority}</span>
                </div>
                <h3 className="font-bold mb-2">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  <a href={joinDiscord} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[hsl(var(--nav-theme)/0.1)] border border-[hsl(var(--nav-theme)/0.3)] text-sm hover:bg-[hsl(var(--nav-theme)/0.2)] transition-colors">
                    <MessageCircle className="w-4 h-4" /> Discord <ExternalLink className="w-3 h-3" />
                  </a>
                  <a href={developerGroup} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[hsl(var(--nav-theme)/0.1)] border border-[hsl(var(--nav-theme)/0.3)] text-sm hover:bg-[hsl(var(--nav-theme)/0.2)] transition-colors">
                    Developer Group <ExternalLink className="w-3 h-3" />
                  </a>
                </div>
              </div>
            </div>
